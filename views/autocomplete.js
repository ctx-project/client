...
export default View.extend({
	...
	drawResults: function(selected = 0) {
		...
		this.content.setContent(this.items.map((item, ix) => itemTpl(item, ix == selected)).join(''));
	},
...
function itemTpl(item, selected) {
	return `<div class="item ${selected ? 'selected' : ''}">${item}</div>`;
};
