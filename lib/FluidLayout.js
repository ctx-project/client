...
export default View.extend({
	...
	initialize: function(options) {
		...
		this.size.on('start', () => {
			...
			window.onresize = debounce(() => this.refreshLayers(), 300);
	...
	addLayer: function(layer) {
		...
		if(layer.name == 'mainPrev') l(_layer.absCorners);
	...
	layoutLayer: function(layer, quick) {
		...
		if(layer.packer)
			layer.packer(layer.items, layer.size).forEach(state => this.moveItem(state.item, state, layer, quick));
		else
	...
	moveItem: function(item, state, layer, quick) {
		...
		if(p) aitem.position$.set([c[0] + p[0], c[1] + p[1], 0], transition);
