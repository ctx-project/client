...
export function pressureArea(items, size, sizer, orderings, ranker, resizer, marginer) {
	var start = performance.now(),
			rank = 0, r,
			trial = items.map(item => {
				var size = sizer(item);
				return {item, size, w: size[0], h: size[1], width: size[0], height: size[1], pressure: 0};
			}),
			best = null;
	...
	for(;;) {
		for(var i = 0; i < orderings.length; i++) {
			...
			if(trial.every(t => t.fit) && (r = ranker(trial)) > rank) {
				rank = r;
				best = trial;
				trial = trial.map(t => ({item: t.item, size: t.size, w: t.w, h: t.h, width: t.w, height: t.h, pressure: t.pressure}));
			}
			
			if(isLate(rank, start)) return toState(best);
			trial.forEach(t => t.fit = undefined);
		}
		
		if(isLate(rank, start) || !resizer(trial)) return toState(best);
	} 
}

function toState(trial) {
	return trial ? trial.map(t => ({item: t.item, position: [t.fit.x, t.fit.y], size: [t.w, t.h], margins: []})) : [];
}
