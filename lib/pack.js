...
export function randomPack(items, size, sizer) {
	reseed();
...
export function fillPack(items, size) {
	return items.map(item => ({
		item, 
		size, 
		position: [0, 0],
...
export function containPack(items, size, sizer, padding = [0, 0]) {
	var	padded = [size[0] - 2 * padding[0], size[1] - 2 * padding[1]];
...
export function horizontalLinePack(items, size, sizer, space) {
	var distance = 0;
	return items.map(item => {
		var isize = sizer(item),
				state = {
...
// 				new Pack(width, height).fit(trial);
